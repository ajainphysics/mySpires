...
class Collaboration
{
    ...
    function __construct($cid) {
        if(gettype($cid) === "string" || gettype($cid) === "integer") {
            ...
            if(!$data->admin || !$data->name || !verify_username($data->admin))
                return;
    ...
    private function load() {
        $result = query("SELECT * FROM collaborations WHERE cid = '{$this->cid}'");
    ...
    function save() {
        ...
        if($this->cid) {
            query("UPDATE collaborations SET name = '{$this->name}', admin = '{$this->admin}', collaborators = '{$collaborators}', pending = '{$pending_collaborators}', suggested = '{$suggested_collaborators}' WHERE cid = {$this->cid}");
        } else {
            query("INSERT INTO collaborations (name, admin, collaborators, pending, suggested) 
    ...
    function add_collaborator($username) {
        if(!$this->loaded) return false;
        if(!verify_username($username)) return false;
