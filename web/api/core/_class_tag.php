...
use function mySpires\users\verify as verify_username;
...
class Tag
{
    ...
    public function __construct($tag, $username = null) {
        if(!verify_username($username, true)) return;
    ...
    private function prepare() {
        ...
        foreach($this->immediate_children as $subtag) {
            $this->subtags[$subtag] = new Tag($subtag, $this->username);
        }
    }
    ...
    public function rename($new_tag) {
        ...
        $new_tag = cleanup($new_tag);

        if(array_key_exists($new_tag, (new User($this->username))->tags()))
            return false;
    ...
    private function properties() {
        if($query = mysqli()->prepare("SELECT * FROM tags WHERE username = ? AND tag = ?")) {
    ...
    private function delete_properties() {
        if($query = mysqli()->prepare("DELETE FROM tags WHERE username = ? AND tag = ?")) {
            $query->bind_param("ss", $this->username, $this->tag);
    ...
    private function set_properties(\stdClass $props = null) {
        ...
        if(!$original_props) {
            if($q = mysqli()->prepare("INSERT INTO tags (username, tag) VALUES (?,?)")) {
        ...
        $props->description = mysqli()->real_escape_string($props->description);

        if($query = mysqli()->prepare("UPDATE tags SET description = ?, starred = ?, shared = ?, visited  = ? WHERE username = ? AND tag = ?")) {
